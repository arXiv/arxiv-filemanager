...
Upload may contain other gzipped archives internall.

"""

#TODO Need to implement uncompress for traditional Unix compress .Z files.
...
# TODO Add logging so we are able to capture additional information during debugging - for now deactivate
debug = 0

def unpack_archive(upload: Upload, archive_path: str):
    """Uppack specified archive and recursively traverse the source directory and unpack any additional
    ...
    # Recursively scan source directory and uplack all archives until there are no more gzipped/tar archives.
    packed_file = 1
    round = 1
    while packed_file:
        # TODO debug logging ("\n*****ROUND " + str(round) + '  Packed: ' + str(packed_file) + '*****\n')
        for root_directory, b, c in os.walk(source_directory):
            # TODO debug logging (f"---> Dir {root_directory} contains the directories {b} and the files {c}")
            for file in c:

                # os.walk provides a list of files with the root directory so we need to build path at each step
                ...
                #print("File is : " + file + " Size: " + str(obj.size) + " File is type: " + obj.type + ":" + obj.type_string + '\n')

                # Tar module is supposed to handle bz2 compressed files (gzip too)
                if ((obj.type == 'tar' or obj.type == 'gzipped') and tarfile.is_tarfile(path)) or obj.type == 'bzip2':
                    ...
                    try:
                        ...
                        upload.add_warning("There were problems opening file '" + obj.public_filepath + "'")
                    ...
                    try:
                        ...
                        for tarinfo in tar:
                            ...
                            # TODO: Need to think about this a little more. Don't really want to flatten
                            # directory structure, but not sure we can just secure basename.
                            #secure = secure_filename(tarinfo.name)
                            #if (secure != tarinfo.name):
                            #    print("\nFile name not secure: " + tarinfo.name + ' (' + secure + ')\n')
                            ...
                            if tarinfo.isreg():
                                ...
                                # Warn about entities we don't want to see in upload archives
                                # We did not check carefully in legacy system and hard links caused bad things to
                                # happen.
                                emsg = ""
                                if tarinfo.issym(): # sym link
                                    upload.add_warning("Symbolic links are not allowed. Removing '" + tarinfo.name + "'.")
                                    ...
                                    upload.add_warning('Character devices are not allowed. Removing ')
                        tar.close()
                ...
                # Hanlde .zip files
                    ...
                    try:
                        ...
                        print("Error processing zip file failed!")
                        upload.add_warning(error_msg_pre + obj.public_filepath + error_msg_suf)
                ...
                # TODO: Handle 'processed' and __MACOSX directories (delete)
